...
if (!isServe) {
  plugins.push(
    new HtmlWebpackPlugin({
      ...
      minify: false,
      inject: false,
      publicPath: `https://www.barrons.com/asset/barrons/graphics-projects/${
        config.slug
      }-${config.uuid}${isDev ? '-dev' : ''}/`,
...
if (config.framework === 'svelte') {
  resolve = {
    ...
    alias: {
      ...
      '$lib/auth': path.resolve(__dirname, 'src/lib/auth'),
    },
    conditionNames: ['svelte', 'browser', 'import', 'module', 'default'],
    mainFields:
      config.svelteVersion === 'svelte3'
  ...
  plugins.push(
    new webpack.NormalModuleReplacementPlugin(
      /node_modules\/barrons-graphics-components\/.*\/\$lib\/components/,
      resource => {
